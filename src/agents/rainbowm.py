...
class ReplayBuffer:
    def __init__(
        ...
        self.acts_buf = torch.zeros([size, action_dim], dtype=torch.long)
...
class Network(nn.Module):
    def __init__(
        ...
        for _ in range(heads):
            ...
            self.advantage_layers.append(NoisyLinear(hidden_size, out_dim * atom_size))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        q_values = torch.sum(dists * self.support.unsqueeze(0), dim=3)
...
class RainbowmAgent(Base):

    def __init__(self, env: VmEnv, config: RainbowmConfig):
        ...
        action_dim = self.env.action_space.nvec[1]

        self.memory = PrioritizedReplayBuffer(obs_dim, n_actions, config.memory_size, config.batch_size, alpha=config.alpha)
        self.use_n_step = True if config.n_step > 1 else False
        if self.use_n_step:
            self.n_step = config.n_step
            self.memory_n = ReplayBuffer(
                obs_dim, n_actions, config.memory_size, config.batch_size, n_step=config.n_step, gamma=config.gamma
            )
        ...
        self.dqn = Network(
            obs_dim, config.hidden_size, n_actions, action_dim, config.atom_size, self.support
        ).to(self.device)
    ...
    def learn(self):
        ...
        for i_episode in pbar:
            ...
            while not done:
                ...
                obs, reward, terminated, truncated, info = self.env.step(action.flatten().cpu().numpy())
    ...
    def _select_action(self, observation: torch.Tensor) -> torch.Tensor:
        return self.dqn(observation).argmax(1)
    ...
    def _compute_dqn_loss(self, samples: Dict[str, np.ndarray], gamma: float) -> torch.Tensor:
        ...
        with torch.no_grad():
            # Double DQN
            next_action = self.dqn(next_state).argmax(2)
            next_dists = self.dqn_target.dist(next_state)
            proj_dists = []
            for i, next_dist in enumerate(next_dists):
                next_dist = next_dist[range(self.config.batch_size), next_action[i, :]]
