...
class VmEnv(gym.Env):
    ...
    def __init__(self, config: Config):
        ...
        self.observation_space = spaces.Box(low=-2, high=self.config.pms+1, shape=(self.config.vms * 3 + self.config.pms * 2,)) 
        self.action_space = spaces.MultiDiscrete(np.full(self.config.vms , self.config.pms + 1))  # Every VM has (PMs or wait action or null action) actions
        self.WAIT_STATUS = -1
        self.NULL_STATUS = -2
    ...
    def get_action_mask(self) -> np.ndarray:
        mask = np.zeros([self.config.vms , self.config.pms + 1], dtype=bool)
        for vm, current_pm in enumerate(self.vm_placement):
            for move_to_pm in range(self.config.pms + 1):
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, dict]:
        action = np.copy(action) - 1 # first action is -1, the wait status
    ...
    def _process_action(self):
        ...
        vms_running = self.vm_placement <= self.WAIT_STATUS
        vms_arrived = np.count_nonzero(vms_running)
        ...
        self.target_cpu_mean = np.sum(self.vm_cpu[vms_running]) / self.config.pms
        if self.config.cap_target_util and self.target_cpu_mean > 1: 
            self.target_cpu_mean = 1.0
        self.target_memory_mean = np.sum(self.vm_memory[vms_running]) / self.config.pms
        if self.config.cap_target_util and self.target_memory_mean > 1: 
            self.target_memory_mean = 1.0
        
        if not (vms_running).any(): # No VMs running
            reward = 0.0
            ...
            target_cpu_var = np.var(self.vm_cpu[vms_running]) 
            target_memory_var = np.var(self.vm_memory[vms_running])
            if target_cpu_var == 0:
                target_cpu_var = 1e-6
    ...
    def reset(self, seed: Optional[int] = None, options=None):
        if seed is None:
            ...
            self.seed(seed)
        # Observable
        self.vm_placement = np.full(self.config.vms, self.NULL_STATUS) 
    ...
    def _accept_vm_requests(self):
        ...
        self.vm_placement[to_accept] = self.WAIT_STATUS
